...
#include "modelcoupled.h"
 ...
 GpxInport::GpxInport(QGraphicsItem * parent, QGraphicsScene * scene, modelPort * data):GpxBlock(parent,
	 scene)
{
	portData = data;
	...
	initPort(GpxConnectionPoint::OutPort);
	QString str;
	portData->owner = this;
...
GpxInport::~GpxInport()
{
	//qDebug("Releasing portdata"); delete portData;
}

void GpxInport::resizeTo(QPointF p)
{
	p = mapFromScene(p);
	switch (resizeCorner) {
	case 1:
...
/*void GpxInport::setPortNumber(int i)
{
	portNumber = i;
	myPortNumber->setText(QString().setNum(i));
	myPolygon.clear();
	myPolygon << QPointF(-(width/2), (height/2)) << QPointF((width/2), (height/2))
		<< QPointF((width/2),-(height/2))<< QPointF(-(width/2),-(height/2))
		<< QPointF(-(width/2), (height/2));
	updatePortsPositions();
	prepareGeometryChange();
	update();
...
void GpxInport::paint(QPainter * painter, const QStyleOptionGraphicsItem *,
		      QWidget *)
{
	painter->
	    setPen(QPen
...
void GpxInport::hoverEnterEvent(QGraphicsSceneHoverEvent * event)
{

}

void GpxInport::updateTextPosition()
{
...
QVariant GpxInport::itemChange(GraphicsItemChange change,
